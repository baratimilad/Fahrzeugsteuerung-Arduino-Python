...
''' 	Gui über Python: Ubuntu 18.04
	...
	4. Python Bluetooth Pakete installieren:
	   ...
	   -> sudo python3 -m pip install pybluez 
		-> klappt nicht. Stattdessen über https://github.com/pybluez/pybluez/issues/180#issuecomment-408235161 
...
testszenarien=glob.glob(path+"/*.csv")
...
class Window(Frame):
	...
	def new_frame(self):
		...
		for t in testszenarien:
			
			tklabel=Button(new_frame,text=t[len(path)+1:-4],command=partial(self.readAndExecuteScenario,t))#todo: change this to regular expression
			tklabel.pack(anchor='w')
		...
		btn_eingabe = Button(new_frame, text="Create", command=self.new_frame_read)
		btn_eingabe.place(x = 280, y = 162) 
		#test_auswahl_leer = Label(new_frame, text="", bg = "grey")
		#test_auswahl_leer.pack(anchor = 'sw')

		
	def senddatatoarduino(self,path):
		csfile=open(path)
		read=csv.reader(csfile,delimiter=',')
		try:
			for row in read:
				print("time: ",row[0],"vy: ",row[1],"vx: ",row[2],"w: ",row[3])
				data =struct.pack('fff',float(row[1]),float(row[2]),float(row[3]))
				self.sock.send(bytes([254]))
				self.sock.send(data)
				time.sleep(float(row[0]))
		except ValueError:
			messagebox.showinfo('check it in ecxel!','CSV-File isnt formatted correctly!')
		except OSError:
			messagebox.showinfo("Wie soll das denn gehen?","Wie willst du eine teststrecke starten, ohne verbunden zu sein?")

		'''threading to keep gu9i responsive'''
	def readAndExecuteScenario(self,path):
		threading.Thread(target=self.senddatatoarduino,args=(path,)).start()
	...
	def new_frame_read(self):
		try:
			...
			csfile=open(newpath,"x",newline='')
			write=csv.writer(csfile,delimiter=',')
			write.writerow([0,0,0,0])
			eingabe.delete(0,'end')
			newButton=Button(new_frame,text=newname,command=partial(self.readAndExecuteScenario,newpath))
			newButton.pack(anchor='w')
		except FileExistsError:
			messagebox.showinfo('Bad idea!','File already exists!')
	...
	def send_info(self):
		...
		vorwärts =struct.pack('ffff',3.2,1,2,-5)
		print(vorwärts)
		aus=("hallo+I")
		ruckwärts=("hallo,I")
		self.sock.send(bytes([254]))
